...
func ScrapeTarget(ctx context.Context, community string, version string, target string, config *config.Module, logger log.Logger) ([]gosnmp.SnmpPDU, error) {
	// Set the options.
	snmp := gosnmp.GoSNMP{}
	snmp.Context = ctx
...
func (c collector) Collect(ch chan<- prometheus.Metric) {
	start := time.Now()
	pdus, err := ScrapeTarget(c.ctx, c.community, c.version, c.target, c.module, c.logger)
	if err != nil {
